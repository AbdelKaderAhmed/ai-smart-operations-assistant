...
function App() {
  ...
  // --- 🔊 نظام النطق المطور (وضوح عالي ومنع توقف) ---
  const speak = (text) => {
  ...
  // --- 🎙️ نظام الاستماع المطور (دقة عالية وتصفية ضوضاء) ---
    ...
    recognition.onresult = (event) => {
      ...
      if (transcript) {
        ...
        if (selectedOp) {
          ...
          setEditData(prev => ({ ...prev, [fieldKey]: (prev[fieldKey] || '') + ' ' + cleaned }));
        } else {
          setInput(prev => prev + (prev ? ' ' : '') + cleaned);
        }
        toast.success(`Captured: "${cleaned}"`);
      }
    };
  ...
  const handleFinalExecute = async () => {
    ...
    try {
      ...
      const msg = `Verified. ${selectedOp.intent.replace('_',' ')} has been executed.`;
      setChat(prev => [...prev, { role: 'bot', content: `✅ ${msg}` }]);
      speak(msg); 
      setSelectedOp(null);
      fetchHistory();
      ...
      <main className="flex-1 flex flex-col relative">
        <header className="h-20 border-b border-slate-800/30 flex items-center px-10">
          <div className="flex items-center gap-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">
            <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></div> Neural Interface Stable
          </div>
        </header>
